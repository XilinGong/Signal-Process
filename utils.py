...
def preparedata(data, labels):
    ...
    for i in range(data.shape[0]):
        data0 = data[i, :]
        hpeaks, _ = find_peaks(data0, distance=50, height=2e-6)
        lpeaks, _ = find_peaks(-data0, distance=50, height=2e-6)
        numpeak = min(len(lpeaks), len(hpeaks))
        peaknum.append(numpeak)
        fitk.append(np.polyfit(data0[:-1], data0[1:], 1)[0])
    peaknum = np.array(peaknum).reshape(-1, 1)
    fitk = np.array(fitk).reshape(-1, 1)
    max_values = np.amax(data, axis=1).reshape(-1, 1)
    min_values = np.amin(data, axis=1).reshape(-1, 1)
    minus = max_values - min_values

    D = labels[:, -1].reshape(-1, 1)
    S = labels[:, -2].reshape(-1, 1)
    R = labels[:, -3].reshape(-1, 1)
    H = labels[:, -4].reshape(-1, 1)

    k = ((max_values - min_values) / (1000 / (peaknum+1))).reshape(-1, 1)
    absk = np.abs(k).reshape(-1, 1)
    data_matrix = np.hstack((D, S, R, H, fitk, min_values, max_values, minus))
    return data_matrix


def show_heatmap(datamatrix, name):
    data_matrixt = datamatrix.T
    correlation_matrix = np.corrcoef(data_matrixt)
    sns.set(style="white")
    plt.figure(figsize=(8, 6))
    sns.heatmap(correlation_matrix, annot=True, cmap="coolwarm",
                xticklabels=["D", "S", "R", "H", 'fitK', 'minv', 'maxv', 'minus'],
                yticklabels=["D", "S", "R", "H", 'fitK', 'minv', 'maxv', 'minus'])
    plt.title(name)
...
def calc_mae(gt, pred):
    mae = np.mean(abs(np.array(gt) - np.array(pred)))
    print('calcmae',mae)
    return mae
